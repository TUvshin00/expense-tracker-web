...
server.post("/record", async (request, response) => {
  const { name, amount, transaction_type, created_at } = request.body;
  try {
    const record = await sql`
      INSERT INTO record (name, amount, transaction_type , created_at  )
      VALUES (${name}, ${amount}, ${transaction_type} , ${created_at})
      RETURNING *
    `;
    response
      .status(201)
      .json({ message: "record message orj chadlaa", record: record[0] });
  } catch (error) {
    console.log(error);
